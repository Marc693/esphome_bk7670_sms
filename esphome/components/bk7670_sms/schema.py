...
CONFIG_SCHEMA = cv.Schema({
    ...
    # IMPORTANT : on utilise output.Output (et non switch.Switch)
    cv.Required(CONF_GPIO_AD): cv.use_id(output.Output),
    cv.Required(CONF_GPIO_HE): cv.use_id(output.Output),
    cv.Required(CONF_GPIO_HG): cv.use_id(output.Output),
    cv.Required(CONF_GPIO_POWERKEY): cv.use_id(output.Output),
})
